...
const softwareItem = (source) => {
  ...
  let logo =
    descriptionField?.screenshots?.[0] ?? source.publiccode?.logo ?? fallback;
...
const getSoftwareCategory = (IPACode) =>
  IPACode ? SOFTWARE_REUSE : SOFTWARE_OPEN;
...
const cropString = (value) =>
  value.length >= 100 ? value.substr(0, 100).concat("...") : value;
